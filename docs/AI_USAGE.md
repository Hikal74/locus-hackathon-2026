...
Pathlight uses Google Gemini in two places, both server-side, both with a single provider and no secondary AI fallback. This document covers both, with the AI advisor (the substantial one) first.
...
## 2. AI rephrasing layer (unchanged in purpose, still Gemini)
...
Both features read `GOOGLE_AI_API_KEY` (required) and `GEMINI_MODEL` (optional, same env var for both call sites, defaults to `gemini-3.6-flash` — live-verified working as of 2026-09-17, see below). The default was deliberately raised from a lite-tier model specifically because the advisor's reasoning task (synthesizing a full profile against multiple database records) is materially harder than rephrasing a handful of already-correct sentences, and a lite-tier model risked shallow, generic output — directly against the advisor's own "no generic advice" requirement. Both call sites share the upgrade since they share the env var; rephrasing quality is a side-benefit, not the reason for the change.
