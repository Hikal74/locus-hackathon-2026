...
- **Vibe Check, home tiles, tiers/report cards, deadlines, Essay Coach, and Interview practice verified live in-browser (2026-09-19)** via more deleted-after-use Playwright scripts: square home tiles (357×357) with results swapping in for the form; tier badges and report cards on cards and Compare; the deadline page including a real `.ics` download (valid calendar, correct event count); Essay Coach and Interview each returned a real Gemini result (quotes shown were verbatim from the draft); the follow-up flow reset correctly; no console errors and no horizontal overflow at 375px. Live testing found and fixed two real bugs: a result rendering below the fold inside a square tile (now a form↔result swap), and `malformed_response` from truncated JSON at `maxOutputTokens: 2048` (raised to 8192, with `finishReason` logged). The free Gemini tier is capped at 20 requests/day per model, so `GEMINI_MODEL` is worth setting per environment.
...
- A denser home page: hero plus three square tool tiles — a working De-Bureaucratizer, Professor Vibe Check, and a Roadmap tile (live "do this right now" + progress once a profile exists, a static teaser otherwise; shares `useRoadmapPlan` with the Roadmap page) — above the Where/Why/What-next strip. De-Bureaucratizer was removed from the NavBar; each tool keeps a full page (`/translate`, `/vibe-check`).
...
- Essay Coach and Interview practice, with the home page's "More tools" link row — see §7.9.
