...
## 13. Onboarding rebuilt as a 4-level effort hierarchy; `/match` methods subsystem removed

**Decision**: Requested directly, in two passes. First, the `/profile` questionnaire was resequenced from a flat 6-step form into 12 steps ordered by an explicit effort hierarchy — Level 1 Quick Demographics (age, grade, native language, language of instruction), Level 2 Preferences & Aspirations (field of study, field-scoped interests, target countries), Level 3 Career Goals & Field Requirements (a field-suggested career path, curated program-priority chips including the research/scholarship preferences), Level 4 Metrics, Exams & Logistics (academic record, standardized exams, language exams, free-text context) — each step tagged with its level in `src/lib/data/onboarding-steps.ts`, the single source of truth for both ordering and level. `StudentProfile` (`src/lib/data/types.ts`) gained matching fields: `nativeLanguage`, `languageOfInstruction`, `careerPath`, `fieldWants`, `curriculumType`, and a split of the old merged `examsCompleted` into `standardizedExamsCompleted` (SAT/ACT/GRE/GMAT/UNT, still scored by `requirementsFit()`) and `languageExamsCompleted` (IELTS/TOEFL — informational only, mirrors how `languageLevel` already handled proficiency scores).

**Iterated down from there, based on direct feedback**: the diagnosis page's "Readiness" section (profile-completeness percentages, not an admission signal) was removed as noise; the standalone "Budget & timing" step was dropped entirely — budget now defaults to a high, non-constraining value and stays editable via the existing What-If control on `/recommendations` — since asking for it up front added friction without changing outcomes for most students; GPA and Age became native `<select>` dropdowns instead of free-typed numbers; a Citizenship & Visa step was tried and then cut for being out of scope of what the deterministic engine or advisor currently do anything with.

**The bigger cut**: the entire `/match` method-picker subsystem (Duels, Rank, Fit Map, AI Interview — see entry §11) was removed, not just its picker page. `src/lib/engine/{personalize,duels}.ts`, `src/lib/ai/{priorities,priorities-schema}.ts`, `POST /api/priorities`, all four `/match/*` route pages, and the now-unused `Slider` UI primitive were deleted along with their tests. This was a deliberate simplification, not a technical necessity — the four methods worked and were tested — but kept as dead-but-correct infrastructure: `useMatchWeights` (`src/lib/store/match-weights.ts`) still exists and every downstream page (Recommendations, Compare, Roadmap, Saved) still reads weights through it, now always the balanced `FIT_WEIGHTS` default, so reintroducing a personalized method later means adding a writer back, not re-plumbing every consumer.

**Roadmap personalization, without an AI call**: entry §12's `buildPortfolioTasks()` was explicitly generic by design ("the same 5 items for every student in that field"). With `careerPath`, `interests`, and `fieldWants` now real structured fields (not free text), that generic-by-design constraint no longer had to hold — the portfolio line's task titles and reasons now substitute the student's own stated interests and career target directly (e.g. "Build a project in Computer Science in AI and robotics — real evidence toward becoming a Software Engineer"), same deterministic-template philosophy as `explain.ts`, just with more structured signal to draw from.

**Files**: `src/lib/data/{types,onboarding-steps,sample-profile}.ts`, `src/components/profile/{types,step-utils}.ts` + `steps/*.tsx` (12 files), `src/app/profile/page.tsx` (rewritten orchestrator), `src/lib/ai/{advisor-schema,advisor-prompt}.ts`, `src/lib/engine/{scoring,diagnosis,roadmap}.ts`, `src/app/diagnosis/page.tsx`, `src/lib/store/match-weights.ts` (simplified), `src/app/recommendations/page.tsx` (dropped the method label + "Refine matches" link). Deleted: `src/app/match/**`, `src/app/api/priorities/`, `src/lib/engine/{personalize,duels}.ts` + tests, `src/lib/ai/{priorities,priorities-schema}.ts` + tests, `src/components/ui/Slider.tsx`.

**Judge explanation (20s)**: "We reordered the test itself so the easy questions — age, what you're into — come first, and the hardest ones — exams, GPA — come last, so nobody bails halfway through. We also cut a whole feature: four different ways to weight your matches turned out to be more choice than value, so now it's just one balanced default, and the code for the other three still exists in git history if that judgment call turns out wrong."

**Limitations**: Removing the 4 match methods removes a documented, tested personalization capability, not just UI — `docs/RECOMMENDATION_ENGINE.md`/`docs/PROJECT_MASTER_GUIDE.md` were updated to describe the current (simpler) state, but this is a real capability regression versus entry §11's rebuild, kept reversible via git rather than actually reversed. Citizenship/visa and campus-size preferences remain modeled in `StudentProfile` but uncollected by any onboarding step — same "known gap, not hidden" pattern as `relevantSubjects` already had.

---

## 14. Third research pass: real Arts program data added

**Reason this happened**: a student picking "Arts" as their field got zero matches everywhere (Universities, Compare, Roadmap all empty) — not a bug, just the honest consequence of Arts being a documented data gap (`docs/DATA_AND_TRUST.md`) that the deterministic hard-filter (`program.field !== profile.intendedField`) then hard-excluded every program against. Asked directly whether to redo the test with a working field instead, or actually go add real data — the latter was chosen.

**Decision**: Researched real Arts/Design undergraduate programs at all 9 dataset universities via live web search + direct fetches of official admissions/fees pages, following the exact same verification-status policy as the two earlier research passes (`docs/DATA_AND_TRUST.md`'s policy section) — never marking a figure `verified` without an official-page URL, `needs_verification` for secondary-source figures, `demo_data` where nothing was found. Added 7 new programs: MIT (Art and Design B.S., Course 4-B), UC Berkeley (Art Practice B.A.), ASU (Art/Expanded Arts BFA, Herberger Institute), Al-Farabi KazNU (Design B.A.), Tsinghua (Visual Communication Design B.A., Academy of Arts & Design), Peking University (Arts B.A., School of Arts), ShanghaiTech (Industrial Design B.Eng., School of Creativity and Art).

**Honest exclusions, not gaps**: Nazarbayev University and KBTU were directly checked (NU's own admissions program list; KBTU's own school list) and confirmed to have no arts/design bachelor's degree at all — these are real absences, not unresearched corners, and the dataset says so explicitly in each university's `notes`.

**One genuinely new-best-source find**: Tsinghua's own official fees page (`international.join-tsinghua.edu.cn/Admission1/Fees.htm`) states Arts programs bill at a distinct ¥40,000/yr rate — higher than Science/Engineering (¥30,000) or Economics/Management/Law (¥26,000) — which is now the dataset's one `verified` figure among the new entries, and incidentally a more solid source than the CS/Business Tsinghua entries already had.

**Side effect, no code change needed**: `src/app/profile/page.tsx`'s `FIELDS_WITH_PROGRAMS` set is derived from the dataset (`programs.map(p => p.field)`), so the "this build's dataset doesn't have programs in this field yet" warning stopped appearing for Arts automatically. `FieldInterestsStep.tsx`'s per-field interest-tag list (already scoped to the chosen field, see entry on the 4-level onboarding rework) also started showing real tags (visual arts, design, fine arts, etc.) instead of its curated fallback, since that fallback only activates when the dataset has zero programs for a field — Medicine still uses it.

**Files**: `src/lib/data/dataset.ts` (+7 programs), `docs/DATA_AND_TRUST.md`, `README.md`, `docs/PROJECT_MASTER_GUIDE.md` (dataset scope: 26→33 programs, 5→6 fields).

**Judge explanation (20s)**: "Arts used to be a documented gap — zero programs, so picking it as your field gave you nothing. We went and found real Arts programs at 7 of the 9 schools, same sourcing rules as the rest of the dataset: official page or it's labeled unconfirmed. The other two schools genuinely don't offer an arts degree — we checked, that's not us being lazy."

**Limitations**: Medicine remains an unresearched gap (out of scope for this pass). Several of the new tuition figures are `demo_data`/`needs_verification` (KazNU, ShanghaiTech, ASU, PKU, Berkeley) — same spread-of-confidence pattern the rest of the dataset already has, not a special weakness of the arts entries specifically. Currency conversions (Tsinghua's ¥40,000, PKU's ¥26,000) used an approximate ¥7.2/USD rate, not a pinned historical rate for the verification date.
